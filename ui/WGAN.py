...
# MODEL_PATH = os.path.join(DATA_PATH, 'checkpoints/model.ckpt-176400')
MODEL_PATH = os.path.join(DATA_PATH, 'checkpoints/model.ckpt-88200')
...
def Load_Model():
    with tf.get_default_graph().as_default() as graph:
        ...
        features_ = np.load('Data/Model_data/SeqCond.npy')
        real_data = np.load('Data/Model_data/50Real.npy')[:, :50, :, :]
...
def run_Model(graph, saver, config, data_factory):
    points = np.load('Points/points2.npy')  # 输入画好的轨迹
